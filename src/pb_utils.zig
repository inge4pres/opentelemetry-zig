...
pub fn hashIdentifyAttributes(attributes: ?pb_common.KeyValueList) u64 {
    if (attributes) |a| {
...
test "generate map key from attributes" {
    var attrs = pb_common.KeyValueList{ .values = std.ArrayList(pb_common.KeyValue).init(std.testing.allocator) };
    defer attrs.values.deinit();

    try attrs.values.append(pb_common.KeyValue{ .key = .{ .Const = "string_key" }, .value = pb_common.AnyValue{ .value = .{ .string_value = .{ .Const = "some_string" } } } });
    try attrs.values.append(pb_common.KeyValue{ .key = .{ .Const = "bool_key" }, .value = pb_common.AnyValue{ .value = .{ .bool_value = true } } });
    try attrs.values.append(pb_common.KeyValue{ .key = .{ .Const = "integer_key" }, .value = pb_common.AnyValue{ .value = .{ .int_value = 42 } } });
